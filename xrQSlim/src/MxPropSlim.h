...
/************************************************************************
  ...
  MxPropSlim
il  Copyright (C) 1998 Michael Garland.  See "COPYING.txt" for details.
...
il    bool use_color;
    bool use_texture;
    bool use_normals;
il    class edge_info : public MxHeapable
    {
    public:
	MxVertexID v1, v2;
	MxVector target;
il	edge_info(unsigned int D) : target(D) { }
    };
    typedef MxSizedDynBlock<edge_info*, 6> edge_list;
ilil    MxBlock<edge_list> edge_links;	// 1 per vertex
